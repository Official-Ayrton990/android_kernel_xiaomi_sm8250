		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret) {
		DRM_ERROR("Failed to read gamma param!\n");
	}
			   ...
			   const char *buf, size_t count)
{
	...
	ret = 0;
	DRM_INFO("set pcc r_c=%d r_r=%d r_g=%d r_b=%d g_c=%d g_r=%d g_g=%d g_b=%d b_c=%d b_r=%d b_g=%d b_b=%d",
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (buf == NULL) {
		DRM_ERROR("disp_pcc_show is NULL!\n");
		return -EINVAL;
	}
