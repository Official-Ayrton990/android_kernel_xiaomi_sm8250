...
static void mtdoops_add_reason(char *oops_buf, enum kmsg_dump_reason reason, enum mtdoops_log_type type, int index, int nextpage)
{
	...
	if (nextpage > 0)
		ret_len = snprintf(str_buf, 200,
				...
				index, UTS_VERSION, kdump_reason[reason], ts.tm_year+1900, ts.tm_mon, ts.tm_mday, ts.tm_hour, ts.tm_min, ts.tm_sec, log_type[type]);
	else
		ret_len = snprintf(str_buf, 200,
				"\n\n# Index: %d\t\n## Build: %s\t\n## Reason: %s\n### %04ld-%02d-%02d %02d:%02d:%02d\n#### Log type: %s\t\n```\t\n",
				index, UTS_VERSION, kdump_reason[reason], ts.tm_year+1900, ts.tm_mon, ts.tm_mday, ts.tm_hour, ts.tm_min, ts.tm_sec, log_type[type]);
			    ...
			    enum kmsg_dump_reason reason)
{
	...
	if(pmsg_start >= pmsg_cpy_size)
		memcpy(cxt->oops_buf + (ret_len + MTDOOPS_HEADER_SIZE), pmsg_addr + (pmsg_start - pmsg_cpy_size), pmsg_cpy_size);
	else {
		pmsg_rem = pmsg_cpy_size - pmsg_start;
		memcpy(cxt->oops_buf + (ret_len + MTDOOPS_HEADER_SIZE), pmsg_addr + (ramoops_data.pmsg_size - pmsg_rem), pmsg_rem);
		memcpy(cxt->oops_buf + ((ret_len + MTDOOPS_HEADER_SIZE) + pmsg_rem), pmsg_addr, pmsg_start);
	}
